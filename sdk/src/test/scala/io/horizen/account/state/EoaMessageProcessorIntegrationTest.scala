...
class EoaMessageProcessorIntegrationTest
  ...
  def canProcess(): Unit = {
    ...
    usingView(EoaMessageProcessor) { view =>
      ...
      assertTrue("Processor expected to BE ABLE to process message", EoaMessageProcessor.canProcess(msg, view, view.getConsensusEpochNumberAsInt))
      ...
      assertFalse("Processor expected to UNABLE to process message", EoaMessageProcessor.canProcess(msg, view, view.getConsensusEpochNumberAsInt))
      ...
      assertFalse("Processor expected to UNABLE to process message", EoaMessageProcessor.canProcess(contractDeclarationMessage, view, 0))
    }
  }
