...
class SigProofTest {
  ...
  def simpleCheck(): Unit = {
    ...
    val signatures: util.List[Optional[Array[Byte]]] = (keyPairs
      ...
      .map(b => Optional.of(b))
      .take(threshold)
      .toList ++ List(Optional.empty(), Optional.empty()))
      .asJava.asInstanceOf[util.List[Optional[Array[Byte]]]]

    val proofAndQuality = sigCircuit.createProof(wb, mcBlockHash, previousMcBlockHash, publicKeysBytes, signatures, threshold, provingKeyPath)
