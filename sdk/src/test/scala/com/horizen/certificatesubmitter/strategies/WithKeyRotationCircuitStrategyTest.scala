...
class WithKeyRotationCircuitStrategyTest extends JUnitSuite with MockitoSugar {
  ...
  def generateProofTest(): Unit = {
    ...
    Mockito.when(mockedCryptolibCircuit.createProof(ArgumentMatchers.any(), ArgumentMatchers.any(),
      ...
      assertResult(4)(answer.getArgument(8).asInstanceOf[SchnorrKeysSignatures].updatedSigningKeysSkSignatures.length)
      assertResult(4)(answer.getArgument(8).asInstanceOf[SchnorrKeysSignatures].updatedSigningKeysMkSignatures.length)
      assertResult(4)(answer.getArgument(8).asInstanceOf[SchnorrKeysSignatures].updatedMasterKeysSkSignatures.length)
      assertResult(4)(answer.getArgument(8).asInstanceOf[SchnorrKeysSignatures].updatedMasterKeysMkSignatures.length)
      assertResult(4)(answer.getArgument(8).asInstanceOf[SchnorrKeysSignatures].schnorrSigners.length)
      assertResult(4)(answer.getArgument(8).asInstanceOf[SchnorrKeysSignatures].schnorrMasters.length)
      assertResult(4)(answer.getArgument(8).asInstanceOf[SchnorrKeysSignatures].newSchnorrSigners.length)
      assertResult(4)(answer.getArgument(8).asInstanceOf[SchnorrKeysSignatures].newSchnorrMasters.length)
      assertResult(2L)(answer.getArgument(9).asInstanceOf[Long])
      assertResult(Optional.empty())(answer.getArgument(10).asInstanceOf[Optional[Integer]])
      assertResult(2)(answer.getArgument(11).asInstanceOf[Integer])
      assertResult(32)(answer.getArgument(12).asInstanceOf[Array[Byte]].length)
  ...
  def getMessageToSignTest(): Unit = {
    ...
    Mockito.when(mockedCryptolibCircuit.generateMessageToBeSigned(ArgumentMatchers.any(), ArgumentMatchers.any(),
      ...
      assertResult(1)(answer.getArgument(6).asInstanceOf[java.util.ArrayList[Object]].size())
