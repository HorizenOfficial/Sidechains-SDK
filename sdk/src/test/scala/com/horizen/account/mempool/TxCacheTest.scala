...
class TxCacheTest
  ...
  def testAdd(): Unit = {
    ...
    assertEquals("wrong size in slot", 0, txCache.getSizeInSlots)
    assertEquals("wrong non exec size in slot", 0, txCache.getNonExecSizeInSlots)
    ...
    assertEquals("wrong size in slot", 1, txCache.getSizeInSlots)
    assertEquals("wrong non exec size in slot", 0, txCache.getNonExecSizeInSlots)
    ...
    assertEquals("wrong size in slot", 3, txCache.getSizeInSlots)
    assertEquals("wrong non exec size in slot", 2, txCache.getNonExecSizeInSlots)
    assertTrue("Oldest is not initialized", txCache.getOldestTransaction().isDefined)
  ...
  def testRemove(): Unit = {
    ...
    assertEquals("wrong size in slot", 5, txCache.getSizeInSlots)
    assertEquals("wrong non exec size in slot", 4, txCache.getNonExecSizeInSlots)
    assertEquals(firstTx, txCache.getOldestTransaction().get)
    assertEquals(thirdTx, txCache.getYoungestTransaction().get)

    txCache.remove(ModifierId @@ firstTx.id)
    assertEquals("wrong size", 2, txCache.size)
    assertEquals("wrong size in slot", 4, txCache.getSizeInSlots)
    assertEquals("wrong non exec size in slot", 4, txCache.getNonExecSizeInSlots)
    ...
    assertEquals("wrong size", 2, txCache.size)
    assertEquals("wrong size in slot", 2, txCache.getSizeInSlots)
    assertEquals("wrong non exec size in slot", 1, txCache.getNonExecSizeInSlots)
    assertEquals(firstTx, txCache.getOldestTransaction().get)
    assertEquals(thirdTx, txCache.getYoungestTransaction().get)
    ...
    assertEquals("wrong size", 2, txCache.size)
    assertEquals("wrong size in slot", 4, txCache.getSizeInSlots)
    assertEquals("wrong non exec size in slot", 3, txCache.getNonExecSizeInSlots)
    assertEquals(firstTx, txCache.getOldestTransaction().get)
    assertEquals(secondTx, txCache.getYoungestTransaction().get)
