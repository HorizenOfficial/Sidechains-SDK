    ...
    with SidechainTypes
{
  ...
  def mainFlowTest() : Unit = {
    ...
    assertTrue("Update(insert) must be successful.",
      sidechainStateStorage.update(version1, withdrawalEpochInfo, (bList1 ++ bList2).toSet, Set(), Seq(), false, Seq(),  consensusEpoch).isSuccess)
    ...
    assertTrue("Update(delete) operation must be successful.",
      sidechainStateStorage.update(version2, withdrawalEpochInfo, Set(),
        boxIdsToRemoveSet, Seq(), false, Seq(), consensusEpoch).isSuccess)
  ...
  def forgerStakesFlow(): Unit = {
    ...
    assertTrue("Update(insert) must be successful.",
      sidechainStateStorage.update(mod1Version, withdrawalEpochInfo, forgerBoxList.toSet, Set(), Seq(), false,
    ...
    assertTrue("Update consensus epoch must be successful.",
      sidechainStateStorage.update(mod3Version, withdrawalEpochInfo, Set(), Set(), Seq(), false, Seq(), nextConsensusEpoch).isSuccess)
  ...
  def withdrawalRequestsFlow() : Unit = {
    ...
    assertTrue("Update(insert) must be successful.",
      sidechainStateStorage.update(mod2Version, mod2WithdrawalEpochInfo, Set(), Set(), newWithdrawalRequestsList, false,
        Seq(), consensusEpoch
      ).isSuccess
    )
    ...
    assertEquals("Storage expected to have different withdrawal requests boxes applied.",
      withdrawalRequestsList ++ newWithdrawalRequestsList, sidechainStateStorage.getWithdrawalRequests(mod1WithdrawalEpochInfo.epoch))
  ...
  def testExceptions() : Unit = {
    ...
    assertFalse("Remove operation of non-existent item must not throw exception.",
      sidechainStateStorage.update(version1, withdrawalEpochInfo, Set(), bList1.map(b => new ByteArrayWrapper(b.id())), Seq(), false, Seq(), intToConsensusEpochNumber(0)).isFailure)
  }
}
