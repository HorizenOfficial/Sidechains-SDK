...
class ConsensusValidatorOmmersTest
  ...
  def emptyOmmersValidation(): Unit = {
    ...
    Try {
      consensusValidator.verifyOmmers(verifiedBlock, currentFullConsensusEpochInfo, previousFullConsensusEpochInfo, parentId, parentInfo, history)
    } match {
      case Success(_) =>
  ...
  def sameEpochOmmersValidation(): Unit = {
    ...
    Try {
      currentEpochConsensusValidator.verifyOmmers(verifiedBlock, currentFullConsensusEpochInfo, previousFullConsensusEpochInfo, parentId, parentInfo, history)
    } match {
      case Success(_) =>
    ...
    Try {
      forgerBoxFailConsensusValidator.verifyOmmers(verifiedBlock, currentFullConsensusEpochInfo, previousFullConsensusEpochInfo, parentId, parentInfo, history)
    } match {
    ...
    val vrfFailConsensusValidator = new ConsensusValidator {
      // always successful
      override def verifyVrfProofAndHash(history: SidechainHistory, header: SidechainBlockHeader, message: VrfMessage): Unit = throw vrfException
      // always fail
      override private[horizen] def verifyForgerBox(header: SidechainBlockHeader, stakeConsensusEpochInfo: StakeConsensusEpochInfo): Unit = {}
    }

    Try {
      vrfFailConsensusValidator.verifyOmmers(verifiedBlock, currentFullConsensusEpochInfo, previousFullConsensusEpochInfo, parentId, parentInfo, history)
    } match {
      case Success(_) => jFail("Block with no ommers expected to be invalid.")
    ...
    // Test 4: Valid ommers with valid subommers in correct order from the same epoch as VerifiedBlock
  ...
  def previousEpochOmmersValidation(): Unit = {
    ...
    Try {
      previousEpochConsensusValidator.verifyOmmers(verifiedBlock, currentFullConsensusEpochInfo, previousFullConsensusEpochInfo, parentId, parentInfo, history)
    } match {
      case Success(_) =>
  ...
  def switchingEpochOmmersValidation(): Unit = {
    ...
    */
    ...
    Mockito.when(history.calculateNonceForNonGenesisEpoch(
      ...
      ArgumentMatchers.any[ListBuffer[(VrfProof, VrfProofHash, ConsensusSlotNumber)]])).thenAnswer(answer => {
      val lastBlockIdInEpoch: ModifierId = answer.getArgument(0)
      val lastBlockInfoInEpoch: SidechainBlockInfo = answer.getArgument(1)
      val acc: ListBuffer[(VrfProof, VrfProofHash, ConsensusSlotNumber)] = answer.getArgument(2)
      ...
      val expectedAcc: ListBuffer[(VrfProof, VrfProofHash, ConsensusSlotNumber)] = ListBuffer(
        (null, null, ConsensusSlotNumber @@ 5)
      )
    ...
    Try {
      switchedEpochConsensusValidator.verifyOmmers(verifiedBlock, currentFullConsensusEpochInfo, previousFullConsensusEpochInfo, parentId, parentInfo, history)
    } match {
      case Success(_) =>
    ...
    Mockito.when(history.calculateNonceForNonGenesisEpoch(
      ...
      assertEquals("On calculate nonce: lastBlockInfoInEpoch is different", parentInfo, lastBlockInfoInEpoch)
      val expectedAcc: ListBuffer[(VrfProof, VrfProofHash, ConsensusSlotNumber)] = ListBuffer(
        (null, null, ConsensusSlotNumber @@ 3),
        (null, null, ConsensusSlotNumber @@ 4)
      )
      assertEquals("On calculate nonce: acc is different", expectedAcc, acc)
  ...
  def switchingEpochOmmersWithSubOmmersValidation(): Unit = {
    ...
    Mockito.when(history.calculateNonceForNonGenesisEpoch(
      ...
      val expectedAcc: ListBuffer[(VrfProof, VrfProofHash, ConsensusSlotNumber)] = ListBuffer(
        (null, null, ConsensusSlotNumber @@ 4),
        (null, null, ConsensusSlotNumber @@ 6)
      )
      assertEquals("On calculate nonce: acc is different", expectedAcc, acc)
