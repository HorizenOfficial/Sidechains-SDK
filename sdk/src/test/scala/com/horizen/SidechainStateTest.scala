...
import com.horizen.certificatesubmitter.keys.{CertifiersKeys, KeyRotationProof, KeyRotationProofType}
...
import com.horizen.transaction.{BoxTransaction, KeyRotationTransaction, OpenStakeTransaction, RegularTransaction}
    ...
    with SidechainTypesTestsExtension
{
  ...
  def getKeyRotationTransaction(boxesWithSecretToOpen: (ZenBox,PrivateKey25519), typeOfKey: KeyRotationProofType.KeyRotationProofType, keyIndex: Int, newKeySecret: SchnorrSecret, oldSigningKeySecret: SchnorrSecret, oldMasterKeySecret: SchnorrSecret, wrongNewKey: Boolean = false): KeyRotationTransaction = {
    ...
    KeyRotationTransaction.create(from, getPrivateKey25519List(1).get(0).publicImage(), 0, typeOfKey.id, keyIndex, newKeySecret.publicImage(), oldSigningKeySignature, newMasterKeySignature, newKeySignature)
  }
  ...
  def keyRotationTransactionTest(): Unit = {
    ...
    Mockito.when(mockedStateStorage.getActualCertifiersKeys(0)).thenReturn(Some(certifiersKeys))

    val newSigningKey = SchnorrKeyGenerator.getInstance().generateSecret("newKey1".getBytes())

    var keyRotationTransaction = getKeyRotationTransaction((boxList.head.asInstanceOf[ZenBox], secretList.head), KeyRotationProofType.SigningKeyRotationProofType, 4, newSigningKey, signingKeys.get(0), masterKeys.get(0))
    ...
    assertFalse("Transaction validation must fail.",
      tryValidate.isSuccess)
    assertTrue(tryValidate.failed.get.getMessage.equals("Key index in KeyRotationTransaction is out of range!"))

    // Test with wrong old signing proof
    keyRotationTransaction = getKeyRotationTransaction((boxList.head.asInstanceOf[ZenBox], secretList.head), KeyRotationProofType.SigningKeyRotationProofType, 0, newSigningKey, signingKeys.get(1), masterKeys.get(0))

    tryValidate = sidechainState.validate(keyRotationTransaction.asInstanceOf[SidechainTypes#SCBT])
    assertFalse("Transaction validation must fail.",
      tryValidate.isSuccess)
    assertTrue(tryValidate.failed.get.getMessage.equals("Signing key signature in KeyRotationTransaction is not valid!"))

    // Test with wrong old master proof
    keyRotationTransaction = getKeyRotationTransaction((boxList.head.asInstanceOf[ZenBox], secretList.head), KeyRotationProofType.SigningKeyRotationProofType, 0, newSigningKey, signingKeys.get(0), masterKeys.get(1))

    tryValidate = sidechainState.validate(keyRotationTransaction.asInstanceOf[SidechainTypes#SCBT])
    assertFalse("Transaction validation must fail.",
      tryValidate.isSuccess)
    assertTrue(tryValidate.failed.get.getMessage.equals("Master key signature in KeyRotationTransaction is not valid!"))

    // Test with wrong old new proof
    keyRotationTransaction = getKeyRotationTransaction((boxList.head.asInstanceOf[ZenBox], secretList.head), KeyRotationProofType.SigningKeyRotationProofType, 0, newSigningKey, signingKeys.get(0), masterKeys.get(0), true)

    tryValidate = sidechainState.validate(keyRotationTransaction.asInstanceOf[SidechainTypes#SCBT])
    assertFalse("Transaction validation must fail.",
      tryValidate.isSuccess)
    assertTrue(tryValidate.failed.get.getMessage.equals("New key signature in KeyRotationTransaction is not valid!"))

    // POSITIVE TESTS

    // Test with a valid transaction
    keyRotationTransaction = getKeyRotationTransaction((boxList.head.asInstanceOf[ZenBox], secretList.head), KeyRotationProofType.SigningKeyRotationProofType, 0, newSigningKey, signingKeys.get(0), masterKeys.get(0))
    tryValidate = sidechainState.validate(keyRotationTransaction.asInstanceOf[SidechainTypes#SCBT])
