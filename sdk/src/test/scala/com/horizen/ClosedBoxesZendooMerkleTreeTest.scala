...
class ClosedBoxesZendooMerkleTreeTest extends ClosedBoxPositionMocker with TempDirectoriesCreator
{
  private def checkEmptyTree(closedBoxesZendooMerkleTree: ClosedBoxesZendooMerkleTree): Unit = {
    ...
    assertTrue(closedBoxesZendooMerkleTree.findFirstIncompatibleTransaction(Seq(emptyTransaction)).isEmpty)
    ...
    assertTrue(closedBoxesZendooMerkleTree.findFirstIncompatibleTransaction(Seq(firstTransaction)).isEmpty)
    ...
    assertFalse(closedBoxesZendooMerkleTree.findFirstIncompatibleTransaction(Seq(incompatibleForEmptyTreeTransaction)).isEmpty)

    //check many transaction on empty tree
    val secondTransaction = createMockedTransaction(Seq(3), Seq(4))
    assertTrue(closedBoxesZendooMerkleTree.findFirstIncompatibleTransaction(Seq(firstTransaction, secondTransaction)).isEmpty)
    assertFalse(closedBoxesZendooMerkleTree.findFirstIncompatibleTransaction(Seq(secondTransaction)).isEmpty)
  ...
  def simpleTest(): Unit = {
    ...
    assertTrue(closedBoxesZendooMerkleTree.findFirstIncompatibleTransaction(firstBlockTxs).isEmpty)
    ...
    assertTrue(closedBoxesZendooMerkleTree.findFirstIncompatibleTransaction(txsWithRemovedOccupiedPositionsForBlock1).isEmpty)
    ...
    assertFalse(closedBoxesZendooMerkleTree.findFirstIncompatibleTransaction(firstBlockTxs).isEmpty)
    ...
    assertTrue(closedBoxesZendooMerkleTree.findFirstIncompatibleTransaction(secondBlockTxs).isEmpty)
    ...
    assertTrue(closedBoxesZendooMerkleTree.findFirstIncompatibleTransaction(txsWithRemovedOccupiedPositionsForBlock2).isEmpty)
