...
class sc_node_holder_fixter_settings extends CompanionsFixture {
  ...
  def generate_scGenesisBlockHex(): Unit = {
    ...
    val mainchainReferences = mcBlocksSerializer.parse(reader).asScala
    ...
    val mainchainHeaders = mainchainReferences.map(_.header) ++ nextMainchainHeaders

    val mainchainMerkleRootHash: Array[Byte] = if(mainchainHeaders.nonEmpty) {
      val mainchainReferencesDataMerkleRootHash = if(mainchainReferences.isEmpty)
        Utils.ZEROS_HASH
      else
        MerkleTree.createMerkleTree(mainchainReferences.map(_.dataHash).asJava).rootHash()
    ...
    val data = new SidechainBlock(signedBlockHeader, Seq(), mainchainReferences, Seq(), Seq(), companion)
