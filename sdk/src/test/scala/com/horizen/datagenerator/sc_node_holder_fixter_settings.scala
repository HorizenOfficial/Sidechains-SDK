...
import com.horizen.utils.{ListSerializer, MerkleTreeFixture}
...
class sc_node_holder_fixter_settings {
  ...
  def generate_scGenesisBlockHex(): Unit = {
    ...
    val owner = PrivateKey25519Creator.getInstance().generateSecret(seed.toString.getBytes)
    val forgerBox: ForgerBox = ForgerBoxFixture.generateForgerBox(seed)
    ...
    val companion = SidechainTransactionsCompanion(new util.HashMap[lang.Byte, TransactionSerializer[SidechainTypes#SCBT]]())
    ...
    val data = new SidechainBlock(parentId, timestamp, mainchainReferences.asScala, Seq(), owner.publicImage(), forgerBox,
