...
class sc_node_holder_fixter_settings extends CompanionsFixture {
  ...
  def generate_scGenesisBlockHex(): Unit = {
    ...
    val mainchainBlockReferences = mcBlocksSerializer.parse(reader).asScala
    ...
    val data = new SidechainBlock(signedBlockHeader, Seq(), mainchainBlockReferences, Seq(), Seq(), companion)
