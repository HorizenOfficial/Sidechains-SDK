...
class MainchainNodeChannelImplTest extends JUnitSuite with MockitoSugar {
      ...
      val thread = new Thread {
        override def run() {
          ...
          p.complete(Success(TopQualityCertificateResponsePayload(MempoolTopQualityCertificateInfo(Some(expectedMempoolCertHash),
            Some(expectedMempoolCertRawCert), Some(expectedMempoolCertQuality), Some(expectedMempoolCertFee)),
            ChainTopQualityCertificateInfo(Some(expectedChainCertHash), Some(expectedChainCertRawCert), Some(expectedChainCertQuality)))))
        }
      }
      thread.start()
      p.future
    ...
    Mockito.when(mockedCommunicationClient.sendRequest[RequestPayload, ResponsePayload](
      ...
      val thread = new Thread {
        override def run() {
          ...
          p.complete(Success(TopQualityCertificateResponsePayload(MempoolTopQualityCertificateInfo(Some(expectedMempoolCertHash),
            Some(expectedMempoolCertRawCert), Some(expectedMempoolCertQuality), Some(expectedMempoolCertFee)),
            ChainTopQualityCertificateInfo(None, None, None))))
        }
      }
      thread.start()
      p.future
  ...
  def getChainTopQualityCertificates(): Unit = {
    ...
    val expectedChainCertQuality = 20
    ...
    Mockito.when(mockedCommunicationClient.sendRequest[RequestPayload, ResponsePayload](
      ...
      val thread = new Thread {
        override def run() {
          ...
          p.complete(Success(TopQualityCertificateResponsePayload(MempoolTopQualityCertificateInfo(None, None, None, None),
            ChainTopQualityCertificateInfo(Some(expectedChainCertHash),
