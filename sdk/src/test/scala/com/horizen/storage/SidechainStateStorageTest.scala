    ...
    with SidechainTypes
{
  ...
  def testUpdate(): Unit = {
    ...
    tryRes = stateStorage.update(version, withdrawalEpochInfo, Set(boxList.head),
      Set(new ByteArrayWrapper(boxList(2).id())), Seq(), consensusEpoch, Seq(), blockFeeInfo, None, false, new Array[Int](0), 0)
    ...
    tryRes = stateStorage.update(version, withdrawalEpochInfo, Set(box),
      Set(new ByteArrayWrapper(boxList(3).id())), Seq(), consensusEpoch, Seq(), blockFeeInfo, None, false, new Array[Int](0), 0)
