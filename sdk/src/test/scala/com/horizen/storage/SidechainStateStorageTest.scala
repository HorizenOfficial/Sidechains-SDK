    ...
    with SidechainTypes
{
  ...
  def testUpdate(): Unit = {
    ...
    tryRes = stateStorage.update(version, withdrawalEpochInfo, Set(boxList.head), Set(boxList(2).id()), Seq(), false, forgingStakesToAppendSeq, consensusEpoch)
    ...
    tryRes = stateStorage.update(version, withdrawalEpochInfo, Set(box), Set(boxList(3).id()), Seq(), false, Seq(), consensusEpoch)
