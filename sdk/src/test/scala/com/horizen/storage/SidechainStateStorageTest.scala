    ...
    with SidechainTypes
{
  ...
  def testUpdate: Unit = {
    ...
    tryRes = stateStorage.update(version, withdrawalEpochInfo, Set(boxList.head), Set(boxList(2).id()), Set(), false)
    ...
    tryRes = stateStorage.update(version, withdrawalEpochInfo, Set(box), Set(boxList(3).id()), Set(), false)
