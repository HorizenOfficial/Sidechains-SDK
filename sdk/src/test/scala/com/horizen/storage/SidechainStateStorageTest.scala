...
import com.horizen.utils.{ByteArrayWrapper, BytesUtils, Pair, WithdrawalEpochInfo, WithdrawalEpochInfoSerializer}
    ...
    with SidechainTypes
{
  ...
  def testUpdate(): Unit = {
    ...
    toUpdate.add(new Pair(new ByteArrayWrapper(stateStorage.forgingStakesInfoKey),
      new ByteArrayWrapper(BytesUtils.fromHexString("00"))))
    toUpdate.add(new Pair(new ByteArrayWrapper(stateStorage.forgingStakesAmountKey),
      new ByteArrayWrapper(Longs.toByteArray(0L))))
    ...
    tryRes = stateStorage.update(version, withdrawalEpochInfo, Set(boxList.head), Set(boxList(2).id()), Set(), Seq(), intToConsensusEpochNumber(0))
    ...
    tryRes = stateStorage.update(version, withdrawalEpochInfo, Set(box), Set(boxList(3).id()), Set(), Seq(), intToConsensusEpochNumber(0))
