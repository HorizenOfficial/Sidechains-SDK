...
class SidechainBlockInfoTest extends JUnitSuite with SidechainBlockInfoFixture {
  ...
  val vrfProofHash: VrfProofHash = VrfGenerator.generateProofHash(height)
  val lastBlockIdInPreviousConsensusEpoch: ModifierId = parentId

  @Test
  def creation(): Unit = {
    val clonedParentId: ModifierId = bytesToId(idToBytes(parentId))
    val info: SidechainBlockInfo = SidechainBlockInfo(height, score, parentId, timestamp, semanticValidity, mainchainHeaderHashes, mainchainReferenceDataHeaderHashes, withdrawalEpochInfo, vrfProofHash, lastBlockIdInPreviousConsensusEpoch)
