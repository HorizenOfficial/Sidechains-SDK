...
class WalletBoxTest extends JUnitSuite with BoxFixture {
  ...
  def WalletBox_CreationTest() {
    val box = getZenBox(getPrivateKey25519("seed1".getBytes(StandardCharsets.UTF_8)), 1, 100)
  ...
  def WalletBox_ComparisonTest(): Unit = {
    ...
    // Test 2: change box
    walletBox2 = new WalletBox(
      getZenBox(getPrivateKey25519("seed2".getBytes(StandardCharsets.UTF_8)), 2, 200),
      bytesToId(new Array[Byte](32)),
      10000)
