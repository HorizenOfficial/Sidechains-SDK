...
object ForkUtil {
  ...
  def selectNetwork[T <: ForkActivation, F](network: String, forks: Map[T, F]): Map[Int, F] = {
   ...
   */
  private def validateForkActivation[T <: ForkActivation, F](network: String, forks: Map[T, F]): Option[F] = {
   ...
   */
  def validate[T <: ForkActivation, F](forks: Map[T, F]): Map[T, F] = {
