    ...
    withBasicAuth {
      _ =>
        entity(as[ReqCreateRedeemMessage]) { body =>
          ...
          if (Sc2ScUtils.isActive(ForkManager.getOptionalSidechainFork[Sc2ScFork](sidechainStateStorage.getConsensusEpochNumber.getOrElse(0)))) {
            ...
            ApiResponseUtil.toResponse(GenericSc2ScApiError("Cannot create redeem message if sc2sc feature is not active"))
          }
        }
    }
  }
...
object Sc2scApiErrorResponse {
  case class GenericSc2ScApiError(description: String, exception: JOptional[Throwable] = JOptional.empty()) extends ErrorResponse {
