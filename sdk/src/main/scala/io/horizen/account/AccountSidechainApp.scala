  ...
  )
{
  ...
  if(sidechainSettings.websocketServer.wsServer) {
    val webSocketServerActor: ActorRef = WebSocketAccountServerRef(nodeViewHolderRef, rpcProcessor, sidechainSettings.websocketServer.wsServerPort, sidechainSettings.websocketServer.wsServerAllowedOrigins)
  }
