...
class WithdrawalEpochValidator(params: NetworkParams) extends HistoryBlockValidator {
  ...
  private def validateBlock(block: SidechainBlock, history: SidechainHistory): Try[Unit] = Try {
    ...
    val backwardTransferCertificateCount = block.mainchainBlocks.flatMap(_.backwardTransferCertificate).size

    if (backwardTransferCertificateCount > 1)
      throw new IllegalArgumentException("Sidechain block must contain 0 or 1 backward transfer certificate, but contains % certificates.".format(backwardTransferCertificateCount))
  }
}
