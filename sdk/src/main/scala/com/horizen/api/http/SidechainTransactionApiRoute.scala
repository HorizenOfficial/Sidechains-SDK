...
case class SidechainTransactionApiRoute(override val settings: RESTApiSettings,
    ...
    entity(as[ReqCreateCoreTransaction]) { body =>
      withNodeView { sidechainNodeView =>
        ...
        if (inputBoxes.length < body.transactionInputs.size) {
          ...
          val outputs: JList[BoxData[Proposition, NoncedBox[Proposition]]] = new JArrayList()
          body.regularOutputs.foreach(element =>
            outputs.add(new RegularBoxData(
              ...
              new lang.Long(element.value)).asInstanceOf[BoxData[Proposition, NoncedBox[Proposition]]])
          )
          ...
          body.forgerOutputs.foreach(element =>
            outputs.add(new ForgerBoxData(
              ...
              new VRFPublicKey(BytesUtils.fromHexString(element.vrfPubKey))).asInstanceOf[BoxData[Proposition, NoncedBox[Proposition]]]  // TODO: replace with VRFPublicKeySerializer later
            )
          )
          ...
          if(fee < 0) {
            ...
            val unsignedTransaction = new SidechainCoreTransaction(boxIds, outputs, fakeProofs, fee, timestamp, companion.boxesDataCompanion, companion.proofsCompanion)
            ...
            val transaction = new SidechainCoreTransaction(boxIds, outputs, proofs.asJava, fee, timestamp, companion.boxesDataCompanion, companion.proofsCompanion)
  ...
  private def createCoreTransaction(inputBoxesType: Class[_<: Box[_ <: Proposition]],
    ...
    // Add change if need.
    if(inputsTotalAmount > inputsMinimumExpectedAmount)
      outputs.add(new RegularBoxData(changeAddress, inputsTotalAmount - inputsMinimumExpectedAmount).asInstanceOf[BoxData[Proposition, NoncedBox[Proposition]]])
    ...
    new SidechainCoreTransaction(boxIds, outputs, proofs.asJava, fee, timestamp, companion.boxesDataCompanion, companion.proofsCompanion)
  }
}
