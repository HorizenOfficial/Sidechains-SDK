...
abstract class AbstractState[
  ...
  def crossChainMessages(withdrawalEpoch: Int): Seq[CrossChainMessage]
  ...
  protected def validateTopQualityCertificateForSc2Sc(topQualityCertificate: WithdrawalEpochCertificate,
    ...
    val messageTreeRoot = certificateCustomFields(1).rawData
    val previousCertificateHash = certificateCustomFields(2).rawData

    val expectedPreviousCertificateHash: Array[Byte] = certificate(topQualityCertificate.epochNumber - 1) match {
      case None => FieldElement.createFromLong(0).serializeFieldElement() //TODO: tobeconfirmed: empty field element is like this?
    ...
    val expectedCrosschainMessages = crossChainMessages(certReferencedEpochNumber)
