...
class ForgeMessageBuilder(mainchainSynchronizer: MainchainSynchronizer,
  ...
  def buildForgeMessageForEpochAndSlot(consensusEpochNumber: ConsensusEpochNumber, consensusSlotNumber: ConsensusSlotNumber,  isReindexing: Boolean, timeout: Timeout, forcedTx: Iterable[SidechainTypes#SCBT]): ForgeMessageType = {
    val forgingFunctionForEpochAndSlot: View => ForgeResult = tryToForgeNextBlock(consensusEpochNumber, consensusSlotNumber, isReindexing, timeout, forcedTx)
  ...
  protected def tryToForgeNextBlock(nextConsensusEpochNumber: ConsensusEpochNumber, nextConsensusSlotNumber: ConsensusSlotNumber,  isReindexing: Boolean, timeout: Timeout, forcedTx: Iterable[SidechainTypes#SCBT])(nodeView: View): ForgeResult = Try {
    log.info(s"Try to forge block for epoch $nextConsensusEpochNumber with slot $nextConsensusSlotNumber")

    if (isReindexing) {
      return ForgeFailedForReindex
    }
