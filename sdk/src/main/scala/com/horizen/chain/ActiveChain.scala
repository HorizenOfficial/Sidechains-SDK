...
final class ActiveChain private(private val sidechainCache: ElementsChain[ModifierId, SidechainBlockInfo],
                                private val mainchainCache: ElementsChain[MainchainBlockReferenceHash, MainchainBlockReferenceData],
  ...
  def mcHeightByMcId(mainChainReferenceId: MainchainBlockReferenceHash): Option[Int] = mainchainCache.heightById(mainChainReferenceId).map(_ + mainchainCreationBlockHeightDifference)

  def mcIdByMcHeight(mcHeight: Int): Option[MainchainBlockReferenceHash] = mainchainCache.idByHeight(mcHeight - mainchainCreationBlockHeightDifference)

  // Active chain store
  def heightOfMc: Int = mainchainCache.height + mainchainCreationBlockHeightDifference

  def mcBlockReferenceDataByMcId(mainChainReferenceId: MainchainBlockReferenceHash): Option[MainchainBlockReferenceData] = mainchainCache.dataById(mainChainReferenceId)

  // Mixed data retrieval
  def heightByMcId(mcId: MainchainBlockReferenceHash): Option[Int] = {
    mainchainCache.dataById(mcId).map(_.sidechainHeight)
  }

  def idByMcId(mcId: MainchainBlockReferenceHash): Option[ModifierId] = {
    heightByMcId(mcId).flatMap(sidechainCache.idByHeight)
  }

  def setBestBlock(newBestId: ModifierId, newBestData: SidechainBlockInfo, mainchainParentId: Option[MainchainBlockReferenceHash]): Unit = {
    if (height == 0) {
  ...
  private def setGenesisBlock(genesisBlockId: ModifierId, genesisBlockInfo: SidechainBlockInfo, mainchainParentId: Option[MainchainBlockReferenceHash]): Unit = {
  ...
  private def setNonGenesisBlock(newBestId: ModifierId, newBestInfo: SidechainBlockInfo, givenMainchainParentId: Option[MainchainBlockReferenceHash]): Unit = {
  ...
  private def addToBothStorages(newTipId: ModifierId, newTipInfo: SidechainBlockInfo, parentForMainchain: Option[MainchainBlockReferenceHash]): Unit = {
  ...
  private def getLastMainchainReferenceBeforeHeight(scHeight: Int): Option[MainchainBlockReferenceHash] = {
    sidechainCache
  ...
  private def buildMainchainBlockReferences(sidechainHeight: Int, mainchainBlockHashes: Seq[MainchainBlockReferenceHash], parentForMainchain: Option[MainchainBlockReferenceHash]) = {
    if (mainchainBlockHashes.nonEmpty) {
      ...
      val bufferForBuildingMainchainBlockReferences = (Seq[(MainchainBlockReferenceHash, MainchainBlockReferenceData)](), parentForMainchain.get)
...
object ActiveChain {
  ...
  def apply(mainchainCreationBlockHeight: Int): ActiveChain = {
    new ActiveChain(new ElementsChain[ModifierId, SidechainBlockInfo](), new ElementsChain[MainchainBlockReferenceHash, MainchainBlockReferenceData](), mainchainCreationBlockHeight: Int)
  }

  // In case of storage with blocks
  def apply(blocksInfoData: ArrayBuffer[(ModifierId, SidechainBlockInfo)], mainchainParent: MainchainBlockReferenceHash, mainchainCreationBlockHeight: Int): ActiveChain = {
