...
case class EthereumConsensusDataReceipt(transactionType: Int,
  ...
  def setLogsBloom(): EthereumConsensusDataReceipt = {
    ...
    val logsBloom = createLogsBloom
    EthereumConsensusDataReceipt(transactionType, status, cumulativeGasUsed, logs, logsBloom)
  }

  def createLogsBloom: Array[Byte] = { // we can create bloom filter out of a log or out of a receipt (taking its log set)
  ...
  override def toString: String = {
    ...
    String.format(s"EvmReceipt (consensus data) { txType=$getTxType, status=$getStatus, cumGasUsed=$cumulativeGasUsed, logs=$logsString, logsBloom=$logsBloomStr}")
  }
}
