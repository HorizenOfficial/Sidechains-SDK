...
object MainchainTxCrosschainOutputAddressUtil {
  ...
  def getAccountAddress(inputAddress: Array[Byte]): Array[Byte] = {
    require(inputAddress.length == Account.ADDRESS_SIZE, s"byte array length ${inputAddress.length} != 32")
    BytesUtils.reverseBytes(inputAddress.take(Account.ADDRESS_SIZE))
  }
}
