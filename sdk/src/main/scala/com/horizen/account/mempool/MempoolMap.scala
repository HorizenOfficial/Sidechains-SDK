...
class MempoolMap(
    ...
    if (!contains(ethTransaction.id)) {
      ...
      expectedNonce.compareTo(ethTransaction.getNonce) match {
        case AddNewExecTransaction =>
          ...
          addNewTransaction(executableTxsPerAccount, ethTransaction, isNotExec = false)
        ...
        case AddOrReplaceNonExecTransaction =>
          ...
          if (existingTxWithSameNonceIdOpt.isDefined) {
            ...
            replaceTransaction(existingTxWithSameNonceId, ethTransaction, nonExecTxsPerAccount, isNotExec = true)
          } else {
            addNewTransaction(nonExecTxsPerAccount, ethTransaction, isNotExec = true)
          }
        case ReplaceExecTransaction =>
          ...
          replaceTransaction(existingTxWithSameNonceId, ethTransaction, executableTxsPerAccount, isNotExec = false)
      }
  ...
  def getAccountSizeInSlots(account: SidechainTypes#SCP): Int = {
    val execSize: Int = executableTxs.get(account).map(txs => txs.values.foldLeft(0) { (sum, txId) => sum + txSizeInSlot(txCache(txId)) }).getOrElse(0)
    val accountSize: Int = nonExecutableTxs.get(account).map(txs => txs.values.foldLeft(execSize) { (sum, txId) => sum + txSizeInSlot(txCache(txId)) }).getOrElse(execSize)
    accountSize
  }
  ...
  private[mempool] def updateAccountWithRevertedNonce(
    ...
    val txsToReinject = new scala.collection.mutable.ListBuffer[(SidechainTypes#SCAT, Boolean)]
...
object MempoolMap {
  ...
  def txSizeInSlot(tx: SidechainTypes#SCAT): Int = bytesToSlot(tx.size())

  def bytesToSlot(numOfBytes: Long): Int = {
