...
import com.horizen.account.state.ForgerStakeMsgProcessor.{
  AddNewStakeCmd,
...
class AccountStateView(
  ...
  lazy val withdrawalReqProvider: WithdrawalRequestProvider = messageProcessors
    .find(_.isInstanceOf[WithdrawalRequestProvider])
    .get
    .asInstanceOf[WithdrawalRequestProvider]
  lazy val forgerStakesProvider: ForgerStakesProvider = messageProcessors
    .find(_.isInstanceOf[ForgerStakesProvider])
    .get
    .asInstanceOf[ForgerStakesProvider]

  // modifiers
  override def applyMainchainBlockReferenceData(
    ...
    refData.sidechainRelatedAggregatedTransaction.foreach(aggTx => {
      aggTx.mc2scTransactionsOutputs().asScala.map {
        case sc: SidechainCreation =>
          ...
          val scOut: MainchainTxSidechainCreationCrosschainOutput =
            sc.getScCrOutput

          val stakedAmount = ZenWeiConverter.convertZenniesToWei(scOut.amount)

          val ownerAddressProposition = new AddressProposition(
            MainchainTxCrosschainOutputAddressUtil
              .getAccountAddress(scOut.address)
          )

          // customData = vrf key | blockSignerKey
          val vrfPublicKey = new VrfPublicKey(
            scOut.customCreationData.take(VrfPublicKey.KEY_LENGTH)
          )
          val blockSignerProposition = new PublicKey25519Proposition(
            scOut.customCreationData.slice(
          ...
          val data = Bytes
            .concat(BytesUtils.fromHexString(AddNewStakeCmd), cmdInput.encode())
          ...
          val message = new Message(
            ...
            stakedAmount,
            BigInteger.ONE
              .negate(), // a negative nonce value will rule out collision with real transactions
            data,
            false
          )

          val returnData =
            forgerStakesProvider.addScCreationForgerStake(message, this)
          log.debug(
            s"sc creation forging stake added with stakeid: ${BytesUtils.toHexString(returnData)}"
          )
        ...
        case ft: ForwardTransfer =>
          ...
          val recipientProposition = new AddressProposition(
            MainchainTxCrosschainOutputAddressUtil
              .getAccountAddress(ftOut.propositionBytes)
          )
          ...
          if (isEoaAccount(recipientProposition.address())) {
            ...
            log.debug(
              s"added FT amount = $value to address=$recipientProposition"
            )
          } else {
            log.warn(
              s"ignored FT to non-EOA account, amount = $value to address=$recipientProposition (the amount was effectively burned)"
            )
  ...
  def applyMessage(
    ...
    new StateTransition(this, messageProcessors, blockGasPool, blockContext)
      .transition(msg)
  }
  ...
  override def applyTransaction(
    ...
    if (!tx.isInstanceOf[EthereumTransaction])
      throw new IllegalArgumentException(
        s"Unsupported transaction type ${tx.getClass.getName}"
      )
    ...
    log.debug(
      s"Returning consensus data receipt: ${consensusDataReceipt.toString()}"
    )
  ...
  override def subBalance(address: Array[Byte], amount: BigInteger): Unit = {
    ...
    amount.compareTo(BigInteger.ZERO) match {
      ...
      case _ =>
        log.debug(
          s"Subtracting $amount to addr ${BytesUtils.toHexString(address)}"
        )
        stateDb.subBalance(address, amount)
    }
  }

  override def getAccountStorage(
      address: Array[Byte],
      key: Array[Byte]
  ): Array[Byte] =
    stateDb.getStorage(address, key, StateStorageStrategy.RAW)

  override def getAccountStorageBytes(
      address: Array[Byte],
      key: Array[Byte]
  ): Array[Byte] =
    stateDb.getStorage(address, key, StateStorageStrategy.CHUNKED)

  override def updateAccountStorage(
      address: Array[Byte],
      key: Array[Byte],
      value: Array[Byte]
  ): Unit =
  ...
  override def updateAccountStorageBytes(
      address: Array[Byte],
      key: Array[Byte],
      value: Array[Byte]
  ): Unit =
  ...
  override def removeAccountStorage(
      address: Array[Byte],
      key: Array[Byte]
  ): Unit =
    stateDb.removeStorage(address, key, StateStorageStrategy.RAW)

  override def removeAccountStorageBytes(
      address: Array[Byte],
      key: Array[Byte]
  ): Unit =
  ...
  override def updateWithdrawalEpochInfo(
      withdrawalEpochInfo: WithdrawalEpochInfo
  ): Unit =
    metadataStorageView.updateWithdrawalEpochInfo(withdrawalEpochInfo)

  override def updateConsensusEpochNumber(
      consensusEpochNum: ConsensusEpochNumber
  ): Unit =
  ...
  def updateBaseFee(baseFee: BigInteger): Unit =
    metadataStorageView.updateBaseFee(baseFee)
  ...
  // getters
  override def withdrawalRequests(
      withdrawalEpoch: Int
  ): Seq[WithdrawalRequest] =
    withdrawalReqProvider.getListOfWithdrawalReqRecords(withdrawalEpoch, this)

  override def certificate(
      referencedWithdrawalEpoch: Int
  ): Option[WithdrawalEpochCertificate] =
  ...
  override def getConsensusEpochNumber: Option[ConsensusEpochNumber] =
    metadataStorageView.getConsensusEpochNumber

  override def getFeePayments(
      withdrawalEpoch: Int,
    ...
    blockToAppendFeeInfo.foreach(blockFeeInfo =>
      blockFeeInfoSeq = blockFeeInfoSeq :+ blockFeeInfo
    )
  ...
  override def getAccountStateRoot: Array[Byte] =
    metadataStorageView.getAccountStateRoot
  ...
  def revertToSnapshot(revisionId: Int): Unit =
