  ...
  override def validate(block: AccountBlock, history: AccountHistory): Try[Unit] = Try {
    ...
    history.getBlockById(block.header.parentId).asScala match {
      case None =>
        ...
        throw new InvalidBaseFeeException(s"No valid parent block with id ${block.header.parentId} found")
      case Some(parentBlock) =>
        val range = parentBlock.header.baseFee.multiply(BigInteger.valueOf(125)).divide(BigInteger.valueOf(1000))
        val lowerBound = baseFee.subtract(range).max(BigInteger.ONE)
        val upperBound = baseFee.add(range).max(BigInteger.ONE)
