    ...
    with SidechainTypes
{
  ...
  private[horizen] def getMainchainBlockHashKey(blockHash: Array[Byte]): ByteArrayWrapper = {
    Utils.calculateKey(Bytes.concat("mainchainBlockHash".getBytes, blockHash))
  }
  ...
  def update(version: ByteArrayWrapper,
    ...
    //for sidechain2Sidehcain we need also the mainchain hash for each certificate
  ...
  def getScTxsCommitment(mainchainHash: Array[Byte]): Option[Array[Byte]] = {
    storage.get(getMainchainBlockHashKey(mainchainHash)).asScala.map(_.data)
  }
