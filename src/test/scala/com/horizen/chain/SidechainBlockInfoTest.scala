...
class SidechainBlockInfoTest extends JUnitSuite with SidechainBlockInfoFixture {
  ...
  def serialization(): Unit = {
    ...
    val serializedInfoTry = SidechainBlockInfoSerializer.parseBytes(bytes)
    ...
    assertTrue("SidechainBlockInfo expected to be not parsed due to broken data.", SidechainBlockInfoSerializer.parseBytes("broken bytes".getBytes).isFailure)
  }
