...
class SidechainHistoryTest extends JUnitSuite with MockitoSugar
  ...
  var customTransactionSerializers: JHashMap[JByte, TransactionSerializer[_ <: Transaction]] = new JHashMap()
  customTransactionSerializers.put(11.toByte, SemanticallyInvalidTransactionSerializer.getSerializer)
  ...
  def synchronizationTest(): Unit = {
    ...
    assertTrue("History 2 continuation Ids for history 1 info expected to be defined.", continuationIds.isDefined)
    assertTrue("History 2 continuation Ids for history 1 info expected to be empty.", continuationIds.get.isEmpty)
    ...
    assertTrue("History 1 continuation Ids for itself info expected to be defined.", continuationIds.isDefined)
    assertTrue("History 1 continuation Ids for itself info expected to be empty.", continuationIds.get.isEmpty)
    ...
    assertTrue("History 1 continuation Ids for history 2 info expected to be defined.", continuationIds.isDefined)
    assertEquals("History 1 continuation Ids for history 2 info expected to be with given size empty.", 1, continuationIds.get.size)
    assertEquals("History 1 continuation Ids for history 2 should contain different data.", history1blockSeq.last.id, continuationIds.get.head._2)
    ...
    assertEquals("History 1 continuation Ids for history 2 info expected to be with given size empty.", 1, continuationIds.get.size)
    assertEquals("History 1 continuation Ids for history 2 should contain different data.", history2blockSeq.last.id, continuationIds.get.head._2)
