...
class MainchainBlockTest extends JUnitSuite {
  ...
  def MainchainBlockTest_SuccessCreationTest(): Unit = {
    ...
    assertEquals("Reserved hash expected to be zero bytes.", BytesUtils.toHexString(MainNetParams.zeroHashBytes), BytesUtils.toHexString(block.get.header.hashReserved))
    assertEquals("SCMap Merkle root hash expected to be zero bytes.", BytesUtils.toHexString(MainNetParams.zeroHashBytes), BytesUtils.toHexString(block.get.header.hashSCMerkleRootsMap))
    ...
    assertEquals("Block equihash solution length is wrong.", MainNetParams.EquihashSolutionLength, block.get.header.solution.length)
    assertTrue("Block expected to be semantically valid", block.get.semanticValidity(MainNetParams))
