...
import com.horizen.websocket.WebSocketChannelOld.ReceivableMessages.{ConnectionError, OpenChannel, ReceiveMessage, SendMessage}
...
class WebSocketChannelOld(webSocketClient :ActorRef, webSocketConfiguration : WebSocketClientSettings)
  ...
  val incoming: Sink[Message, Future[Done]] =
    Sink.foreach[Message] {
      case tm: TextMessage =>
        //handler.onReceiveMessage(tm.getStrictText)
...
object WebSocketChannelRef{
  def props(webSocketClient :ActorRef, webSocketConfiguration : WebSocketClientSettings)
    ...
    Props(new WebSocketChannelOld(webSocketClient, webSocketConfiguration))
