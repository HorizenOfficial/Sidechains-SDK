...
class ChainedData[ID, DATA <: ChainData[ID]](private var lastId: Option[ID] = None,
  ...
  def cutToId(newBestId: ID): Try[Unit] = Try {
    if (height > 0) {
      if (newBestId == data(0).getParentId) {
        ...
        // in common case: cut to id A -> A is stay as required parent for new data
        // cut to the parent of the first element -> parent of the first element is no longer required as parent
        clear()
      }
  ...
  def appendData(newId: ID, newData: DATA): Unit = {
    require(height == 0 || newData.getParentId != bestId, "Try to append block with incorrect parent")
  ...
  private def getLastDataByPredicateForHeight(heightCounter: Integer, p: DATA => Boolean): Option[DATA] = {
    dataByHeight(heightCounter) match {
      case Some(data) => if (p(data)) Some(data) else getLastDataByPredicateForHeight(heightCounter - 1, p)
      case None => None
    }
  }
  ...
  private def getLastIdsForHeight(counter: Int, lastHeight: Int, collectedIds: Seq[ID]): Seq[ID] = {
    require(counter >= 0, "Counter shall not be negative")
