...
case class SidechainStateApi (override val settings: RESTApiSettings,
  ...
  private var boxCompanion : SidechainBoxesCompanion = new SidechainBoxesCompanion(new util.HashMap[java.lang.Byte, BoxSerializer[_ <: Box[_ <: Proposition]]]())
      ...
      withNodeView{ sidechainNodeView =>
        ApiInputParser.parseInput[GetClosedBoxesRequest](body)match {
          case Success(req) =>
            ...
            var scalaClosedBoxesSerialized = allClosedBoxes.asScala.map(box => boxCompanion.toBytes(box))
