...
import com.horizen.api.http.{SidechainApiRoute, SidechainCompositeHttpService}
import scorex.core.api.http.{ApiErrorHandler, ApiRejectionHandler}
...
trait ScorexApplication extends ScorexLogging {
  ...
  //api
  val sidechainApiRoutes: Seq[SidechainApiRoute]
  ...
  lazy val combinedRoute: Route = SidechainCompositeHttpService(actorSystem, sidechainApiRoutes, settings.restApi, swaggerConfig, nodeViewHolderRef).compositeRoute

  def run(): Unit = {
    require(settings.network.agentName.length <= ScorexApplication.ApplicationNameLimit)
