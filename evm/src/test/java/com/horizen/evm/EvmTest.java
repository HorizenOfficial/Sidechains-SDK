...
public class EvmTest extends LibEvmTestBase {
    ...
    public void evmApply() throws Exception {
        ...
        try (var db = new MemoryDatabase()) {
            try (var statedb = new StateDB(db, hashNull)) {
                ...
                result = Evm.Apply(statedb, addr2, contractAddress, null, funcRetrieve, gasLimit, gasPrice, null, new TraceOptions(true, false, false, true));
                assertEquals("", result.evmError);
    ...
    public void brokenCodeExecution() throws Exception {
        ...
        final var gasPrice = BigInteger.valueOf(10);
        final var input =
                bytes("5234801561001057600080fd521683398151915290610073906020808252600c90820190565b60405180910390a2336001600160a01b03");
        ...
        try (var db = new MemoryDatabase()) {
            try (var statedb = new StateDB(db, hashNull)) {
                ...
                statedb.setNonce(addr1, BigInteger.ZERO);
                result = Evm.Apply(statedb, addr1, null, null, input, gasLimit, gasPrice, null, null);
                assertNotNull(result.evmError);
            }
        }
    }
