...
interface ForgerStakesV2 {
    ...
    event WithdrawForgerStake(address indexed owner, bytes32 signPubKey, bytes32 indexed vrf1, bytes1 indexed vrf2, uint256 value);
    ...
    */
    ...
    /*
       ...
       be < maxNumOfEpoch if the current consensus epoch is < (consensusEpochStart + maxNumOfEpoch)
    */
    function rewardsReceived(bytes32 signPubKey, bytes32 vrf1, bytes1 vrf2, uint32 consensusEpochStart, uint32 maxNumOfEpoch) external view returns (uint256[] memory);

    /*
    /  Returns the info of a specific registered forger.
    */
    function getForger(bytes32 signPubKey, bytes32 vrf1, bytes1 vrf2) external view returns (ForgerInfo memory);
    ...
    */
    function getPagedForgersStakesByForger(bytes32 signPubKey, bytes32 vrf1, bytes1 vrf2, int32 startIndex, int32 pageSize) external view returns  (int32, StakeDataDelegator[] memory);
