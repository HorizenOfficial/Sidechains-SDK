...
def check_scparent(parent_scblock_id, scblock_id, sc_node):
    ...
    logging.info("SC Block {0} has parent id {1}.".format(scblock_id, parent_scblock_id))
...
def check_mcheader_presence(mcblock_hash, scblock_id, sc_node):
    ...
    for header in headers:
        if header["hash"] == mcblock_hash:
            logging.info("MC hash {0} is present in SC Block {1} mainchain headers.".format(mcblock_hash, scblock_id))
            return
...
def check_mcreferencedata_presence(mcblock_hash, scblock_id, sc_node):
    ...
    for refData in refDataList:
        if refData["headerHash"] == mcblock_hash:
            logging.info("MC hash {0} is present in SC Block {1} mainchain reference data.".format(mcblock_hash, scblock_id))
            return
...
def check_mcheaders_amount(amount, scblock_id, sc_node):
    ...
    logging.info("SC block {0} contains {1} mainchain headers.".format(scblock_id, amount))
...
def check_mcreferencedata_amount(amount, scblock_id, sc_node):
    ...
    logging.info("SC block {0} contains {1} mainchain reference data.".format(scblock_id, amount))
...
def check_ommers_amount(amount, scblock_id, sc_node):
    ...
    logging.info("SC block {0} contains {1} ommers.".format(scblock_id, amount))
...
def check_ommers_cumulative_score(score, scblock_id, sc_node):
    ...
    logging.info("SC block {0} has cumulative score {1}.".format(scblock_id, score))
...
def check_ommer(ommer_scblock_id, ommer_mcheaders_hashes, scblock_id, sc_node):
    ...
    for ommer in ommers:
        if ommer["header"]["id"] == ommer_scblock_id:
            logging.info("Ommer id {0} is present in SC Block {1} ommers.".format(ommer_scblock_id, scblock_id))
            ...
            for header in ommer_mcheaders:
                if header["hash"] in ommer_mcheaders_hashes:
                    logging.info("MC hash {0} is present in Ommer {1} mainchain headers.".format(header["hash"],
                                                                                          ommer_scblock_id))
                    return
                else:
        ...
        if ommer["header"]["id"] == ommer_scblock_id:
            ...
            for subommer in subommers:
                if subommer["header"]["id"] == subommer_scblock_id:
                    logging.info("Subommer id {0} is present in Ommer {1} in SC Block {2}.".format(subommer_scblock_id,
                                                                                            ommer_scblock_id,
                                                                                            scblock_id))
                    ...
                    for header in subommer_mcheaders:
                        if header["hash"] in subommer_mcheader_hashes:
                            logging.info("MC hash {0} is present in Subommer {1} mainchain headers.".format(header["hash"],
                                                                                                     subommer_scblock_id))
                            return
                        else:
...
def mc_make_forward_transfer(mc_node, sc_node, sc_id, ft_zen_amount, mc_return_address):
    ...
    logging.info("MC Tx with FT created: " + mc_tx_id)
    ...
    logging.info("MC block with FT generated: " + mc_block_hash)
...
def sc_create_forging_stake_mempool(sc_node, stake_zen_amount):
    ...
    if "result" not in res:
        ...
        logging.info("Forget stake created: " + json.dumps(res))
...
def sc_make_withdrawal_request_mempool(mc_node, sc_node, bt_zen_amount):
    ...
    if "result" not in res:
        ...
        logging.info("Coins withdrawn: " + json.dumps(res))
