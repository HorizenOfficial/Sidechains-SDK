...
class SCEvmForgingStakes(AccountChainSetup):
    ...
    def run_test(self):
        ...
        # Initialize the nonce count for each wallet per block and overall
        nonces_per_wallet_per_block = [0] * number_of_wallets
        nonces_per_wallet_overall = [0] * number_of_wallets
        ...
        while stakes_created < number_of_stakes:
            # Check if we've reached the maximum number of nonces for this wallet per block
            if nonces_per_wallet_per_block[wallet_index] < max_nonces_per_wallet:

                # Create a stake
                result = ac_makeForgerStake(sc_node_1, wallets[wallet_index], sc1_blockSignPubKey, sc1_vrfPubKey, convertZenToZennies(forgerStake_amount), nonces_per_wallet_overall[wallet_index])

                if "result" not in result:
                    fail(f"{wallets[wallet_index]} make forger stake {stakes_created} failed for nonce {nonces_per_wallet_overall[wallet_index]}: " + json.dumps(result))
                else:
                    logging.info(f"{wallets[wallet_index]} Forger stake {stakes_created} created: " + json.dumps(result))

                # Update our counts
                stakes_created += 1
                nonces_per_wallet_per_block[wallet_index] += 1
                nonces_per_wallet_overall[wallet_index] += 1
                ...
                if stakes_created % max_transactions_per_block == 0:
                    ...
                    # Reset the nonce count per block for each wallet
                    nonces_per_wallet_per_block = [0] * number_of_wallets
            else:
                # If the current wallet reached its maximum nonce per block, increment the wallet_index to use the next wallet
                wallet_index += 1
