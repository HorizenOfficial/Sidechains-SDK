...
from SidechainTestFramework.scutil import bootstrap_sidechain_nodes, \
    start_sc_nodes, generate_next_blocks, generate_next_block, check_wallet_coins_balance, generate_cert_signer_secrets
...
class SCKeyRotationTest(SidechainTestFramework):
    ...
    def sc_setup_chain(self):
        ...
        if (self.circuit_type == 1):
            sc_creation_version = 2
        else:
            sc_creation_version = 1
        network = SCNetworkConfiguration(SCCreationInfo(mc_node, 100, self.sc_withdrawal_epoch_length,
                                                        ...
                                                        type_of_circuit_number = self.circuit_type,
    ...
    def run_test(self):
        ...
        # If the circuit doesn't support key rotation check that we don't support also the KeyRotationTransaction
        if (self.circuit_type == 1):
            ...
            assert_true("Signing key signature in KeyRotationTransaction is not valid" in response["error"]["detail"])
            ...
            response = http_create_key_rotation_transaction(sc_node, 
                                                ...
                                                automatic_send=True)
            assert_true("error" in response)
            assert_true("Master key signature in KeyRotationTransaction is not valid" in response["error"]["detail"])
            ...
            response = http_create_key_rotation_transaction(sc_node, 
                                                ...
                                                automatic_send=True)
            assert_true("error" in response)
            assert_true("New key signature in KeyRotationTransaction is not valid" in response["error"]["detail"])    
            ...
            response = http_create_key_rotation_transaction(sc_node, 
                                                ...
                                                automatic_send=True)
            assert_true("error" in response)
            assert_true("Key index in KeyRotationTransaction is out of range" in response["error"]["detail"])         
            ...
            # Try to send another KeyRotationTransaction pointing to the same key
            error = False
            try:
                http_create_key_rotation_transaction(sc_node, 
                                                            key_type=0,
