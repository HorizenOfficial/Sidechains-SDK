...
class CertTestUtils(MCTestUtils):
    def __init__(self, datadir, srcdir=None, ps_type = "cob_marlin"):
        if srcdir==None:
            # get the tool path from the env var used for getting zend
            zendPath = os.getenv("BITCOIND", "N.A.")
            if not os.path.isfile(zendPath):
                raise Exception('no such file: ' + zendPath)
            srcdir = os.path.dirname(zendPath)
    ...
    def create_test_proof(self, id, scid, epoch_number, quality, btr_fee, ft_min_amount, end_cum_comm_tree_root, prev_cert_hash = None,
     constant = None, pks = [], amounts = [], custom_fields = [], num_constraints = 1 << 10, segment_size = 1 << 9, keyrot = False):
        circ_type = "cert"
        ...
        args = self._get_args("create", circ_type, constant, keyrot, self.ps_type, params_dir, num_constraints, segment_size, proof_path)
