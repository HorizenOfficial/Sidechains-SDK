...
def send_transactions_per_second(txs_creator_node, txs_creator_addresses, destination_address, tx_amount, start_time, test_run_time,
...
class PerformanceTest(SidechainTestFramework):
    ...
    def txs_creator_send_transactions_per_second_to_addresses(self, tx_amount, txs_creator_nodes, non_creator_nodes, txs_creator_addresses, tps_test):
                ...
                with Pool(initializer=init_globals, initargs=(counter, errors)) as pool:
                    ...
                    for index, sc_node in enumerate(self.sc_nodes_list):
                        if sc_node["tx_creator"]:
                            ...
                            del sc_nodes[index]
                            args.append((self.sc_nodes[index], txs_creator_addresses, destination_address, tx_amount, start_time,
    ...
    def run_test(self):
        ...
        for i in range(len(txs_creators)):
            txs_creator_addresses[txs_creators[i]] = txs_creators[i].wallet_createPrivateKeySecp256k1()["result"]["proposition"]["address"]
            print(txs_creator_addresses[txs_creators[i]])
            forward_transfer_to_sidechain(self.sc_nodes_bootstrap_info.sidechain_id, mc_nodes[0],
                txs_creator_addresses[txs_creators[i]], ft_amount, mc_return_address)
        self.sc_sync_all()
        ...
        if self.test_type == TestType.Mempool:
            ...
            while not end_test:
                ...
                for creator_node in txs_creators:
                    response = creator_node.rpc_eth_gasPrice()
        ...
        ##########################################################
            ...
            self.txs_creator_send_transactions_per_second_to_addresses(tx_amount, txs_creators, non_txs_creators, txs_creator_addresses, True)

        elif self.test_type == TestType.All_Transactions:
            # 1 thread per txs_creator node sending transactions
            self.txs_creator_send_transactions_per_second_to_addresses(tx_amount, txs_creators, non_txs_creators, txs_creator_addresses, False)
