...
"""
class SCGenerateTestData(SidechainTestFramework):
    ...
    def write_test_data(self, test_data_file, test_data_header, test_data):
        test_data_file.write("------------------------------------------------------------\n")
        test_data_file.write(test_data_header)
        test_data_file.write("\n")
        test_data_file.write("------------------------------------------------------------\n")
    ...
    def run_test(self):
        ...
        #Write test data - sidechain creation transaction and block, containing it.
        sc_info = mc_node.getscinfo()[0]
        creation_block = mc_node.getblock(sc_info["created in block"], False)
        creation_transaction = mc_node.gettransaction(sc_info["creating tx hash"])["hex"]
        self.write_test_data(test_data_file, "Sidechain creation MC block", creation_block)
        self.write_test_data(test_data_file, "Sidechain creation MC transaction", creation_transaction)
        ...
        ft_amount = 10
        ft_transaction = mc_node.sc_send(sc_address, ft_amount, self.sc_nodes_bootstrap_info.sidechain_id)
        ...
        #Write test data - sidechain forward transfer transaction and block, containing it.
        ft_block = mc_node.getblock(mcblock_hash1, False)
        ft_transaction = mc_node.gettransaction(ft_transaction)["hex"]
        self.write_test_data(test_data_file, "Sidechain forward transfer MC block", ft_block)
        self.write_test_data(test_data_file, "Sidechain forward transfer MC transaction", ft_transaction)
        ...
        #Write test data - sidechain forward transfer transaction and block, containing it.
        bt_block = mc_node.getblock(we1_2_mcblock_hash, False)
        bt_transaction = mc_node.gettransaction(we0_cert["certid"])["hex"]
        self.write_test_data(test_data_file, "Sidechain backward transfer MC block", bt_block)
        self.write_test_data(test_data_file, "Sidechain backward transfer MC transaction", bt_transaction)
