...
Test:
    For the SC node:
        ...
        - mine 2 more MC block and forge 1 more SC block, check Certificate inclusion into SC block
...
class SCBackwardTransfer(SidechainTestFramework):
    ...
    def sc_setup_nodes(self):
        return start_sc_nodes(1, self.options.tmpdir, extra_args=[['-mc_block_delay_ref', '1']])
    ...
    def run_test(self):
        ...
        we1_3_mcblock_hash = mc_node.generate(1)[0]
        epoch_mc_blocks_left -= 1
        assert_equal(0, mc_node.getmempoolinfo()["size"], "Certificate expected to be removed from MC node mempool.")
        assert_equal(1, len(mc_node.getblock(we1_3_mcblock_hash)["tx"]), "MC block expected to contain 1 transaction.")
        assert_equal(1, len(mc_node.getblock(we1_3_mcblock_hash)["cert"]),
                     "MC block expected to contain 1 Certificate.")
        assert_equal(we0_certHash, mc_node.getblock(we1_3_mcblock_hash)["cert"][0],
                     "MC block expected to contain certificate.")
        logging.info("MC block with withdrawal certificate for epoch 0 = {0}\n".format(
            str(mc_node.getblock(we1_3_mcblock_hash, False))))
        ...
        if "result" not in withdrawCoinsJson:
            ...
            logging.info("Coins withdrawn: " + json.dumps(withdrawCoinsJson))
        ...
        # Generate MC block and verify that certificate is present
        we2_3_mcblock_hash = mc_node.generate(1)[0]
        assert_equal(0, mc_node.getmempoolinfo()["size"], "Certificate expected to be removed from MC node mempool.")
        assert_equal(1, len(mc_node.getblock(we2_3_mcblock_hash)["tx"]), "MC block expected to contain 1 transaction.")
        assert_equal(1, len(mc_node.getblock(we2_3_mcblock_hash)["cert"]),
                     "MC block expected to contain 1 Certificate.")
        assert_equal(we1_certHash, mc_node.getblock(we2_3_mcblock_hash)["cert"][0],
                     "MC block expected to contain certificate.")
        logging.info("MC block with withdrawal certificate for epoch 1 = {0}\n".format(
            str(mc_node.getblock(we2_3_mcblock_hash, False))))
