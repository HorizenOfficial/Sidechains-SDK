...
class SCWsAccountServerTest(AccountChainSetup):
    ...
    def run_test(self):
        ...
        response = json.loads(ws_connection.recv())
        # new block generated by the SC node 2 after the reconnection
        response = json.loads(ws_connection.recv())
        ...
        response = json.loads(ws.sendMessage(ws_connection, SC_NODE1_ID, ws.UNSUBSCRIBE_REQUEST, [new_heads_subscription]))
        response = json.loads(ws.sendMessage(ws_connection, SC_NODE1_ID, ws.UNSUBSCRIBE_REQUEST, [new_heads_subscription2]))
