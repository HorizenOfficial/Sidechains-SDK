...
Test:
    ...
    - Mine 2 MC blocks on MC node 1, sync with MC node 3, then forge SC block respectively, verify MC data inclusion.
    - Mine 3 MC blocks on MC node 2. Connect and synchronize MC nodes 1 and 2. Fork became an active chain.
...
class MCSCEvmForging3(AccountChainSetup):
    ...
    def run_test(self):
        ...
        mcblock_hash4 = mc_node3.generate(1)[0]

        # Sync MC nodes 1 and 3 once
        connect_nodes_bi(self.nodes, 0, 2)
        self.sync_nodes([mc_node1, mc_node3])
        disconnect_nodes_bi(self.nodes, 0, 2)
        # MC Node 1 should replace back fork_mcblock_hash2 Tip with [mcblock_hash1, mcblock_hash2, mcblock_hash3, mcblock_hash4]
        assert_equal(mcblock_hash4, mc_node1.getbestblockhash())
