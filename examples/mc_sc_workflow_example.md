...
Run again the Bootstrapping tool and execute the `genesisinfo` command with the next parameters:
...
3. `signersPublicKeys` - list of Schnorr signer public keys were generated by the command `generateCertificateSignerKey`.
...
5. `signersSecrets` - list of Schnorr secret keys were generated by the command `generateCertificateSignerKey`.
6. `circuitType` - a type of the circuit of the certificate submitter, can be NaiveThresholdSignatureCircuit or NaiveThresholdSignatureCircuitWithKeyRotation
...
To be verified as an owner of the first Forger box and be able to forge new Sidechain blocks, generated secrets from step 3 shall be added as well. Find `genesisSecrets` parameter in `wallet` group of the config file and add your secret keys.
